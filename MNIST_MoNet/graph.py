...
def distance_sklearn_metrics(z, k=4, metric='euclidean'):
    ...
    d.sort()
    d = d[:, 1:k+1]
    angle = np.zeros((len(z), k))
    for i in range(len(z)):
        angle[i,:] = np.arctan2(z[idx[i,:],1] - z[i, 1], z[idx[i,:],0] - z[i, 0])
    return d, angle, idx
...
def distance(dist, idx):
    """Return the distance matrix of a kNN graph."""
    M, k = dist.shape
    ...
    J = idx.reshape(M*k)
    V = dist.reshape(M*k)
    D = scipy.sparse.coo_matrix((V, (I, J)), shape=(M, M))
    ...
    # Non-directed graph.
    bigger = D.T > D
    D = D - D.multiply(bigger) + D.T.multiply(bigger)

    assert D.nnz % 2 == 0
    assert np.abs(D - D.T).mean() < 1e-10
    assert type(D) is scipy.sparse.csr.csr_matrix
    return D
